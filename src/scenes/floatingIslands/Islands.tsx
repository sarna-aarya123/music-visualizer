...
export function Islands({
  ...
  useFrame((state) => {
    ...
    materials.body.uniforms.uEmissive.value = Math.min(
      0.4 + featureFrame.sectionMood * 0.35 + featureFrame.energy * 0.25 + env * 0.85,
      1.95
    );
    materials.canopy.uniforms.uEmissive.value = Math.min(
      0.08 + rhythmState.drumPresence * 0.22 + featureFrame.sectionMood * 0.15 + env * 0.42,
      1.35
    );

    for (const m of Object.values(materials)) {
      (m.uniforms.uCameraPos.value as THREE.Vector3).copy(cam);
      m.uniforms.uRimStrength.value = (m.userData.baseRim as number) * (1 + env * 0.7);
    }
